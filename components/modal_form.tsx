...
async function submitForm(dateRange: RangeProps[], formData: any, submitType: "seller" | "buyer") {
  if (submitType === "seller") {
    console.log("adding passes")
    console.log(dateRange)
    await addPasses(dateRange, formData)
  } else {
    await requestPasses(dateRange, formData)
  }
}
...
export const ModalForm: React.FC<ModalFormProps> = ({open, onClose, dateRange, submitType}) => {
  ...
  const submit = (e: any, dateRange: RangeProps[]) => {
    e.preventDefault()
    console.log(formData)
